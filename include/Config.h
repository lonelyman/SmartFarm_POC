...
constexpr int PIN_WATER_LEVEL_CH1_LOW_SENSOR = 1; // GPIO1 — ถัง CH1 (เปลี่ยนจาก 32 — อยู่ในกลุ่ม reserved 26–37)
constexpr int PIN_WATER_LEVEL_CH2_LOW_SENSOR = 2; // GPIO2 — ถัง CH2 (เปลี่ยนจาก 33 — อยู่ในกลุ่ม reserved 26–37)
...
constexpr uint32_t MIST_MIN_OFF_MS = 10 * 60 * 1000UL; // พักอย่างน้อย 10 นาที

// ════════════════════════════════════════════════════════════
//  SECTION 12 : MIST CONFIRMATION COUNTER
//  ป้องกัน mist เปิด/ปิดจากค่า sensor ที่ข้ามเส้นชั่วคราว
//
//  MIST_CONFIRM_ON_COUNT  : ต้องเห็น sensorWantsOn=TRUE ติดต่อกันกี่รอบ
//                           ถึงจะเปิด mist จริง
//  MIST_CONFIRM_OFF_COUNT : ต้องเห็น sensorWantsOn=FALSE ติดต่อกันกี่รอบ
//                           ถึงจะยกเลิกความต้องการเปิด
//
//  1 รอบ = INPUT_TASK_INTERVAL_MS (ปัจจุบัน 1 วินาที)
//  ตัวอย่าง: ON_COUNT=3 → ต้องเห็นค่าเกินเส้น 3 วินาทีติดต่อกัน
//
//  ⚠️  ใน unit test ใช้ค่า 1 เพื่อให้ test เดิมผ่านโดยไม่ต้องแก้
// ════════════════════════════════════════════════════════════
constexpr uint8_t MIST_CONFIRM_ON_COUNT = 3;  // รอบ — ยืนยันก่อนเปิด
constexpr uint8_t MIST_CONFIRM_OFF_COUNT = 3; // รอบ — ยืนยันก่อนยกเลิก

// ════════════════════════════════════════════════════════════
//  SECTION 13 : TASK CONFIG (FreeRTOS + Serial)
//  - SERIAL_BAUD: baud rate สำหรับ Serial monitor/commands (ไม่ใช่ค่า FreeRTOS โดยตรง)
//  - *_INTERVAL_MS: รอบการทำงานของแต่ละ task
//  - *_STACK: ขนาด stack ของ task (หน่วย: bytes สำหรับ Arduino/ESP32)
// ════════════════════════════════════════════════════════════
constexpr unsigned long SERIAL_BAUD = 115200; // bps — Serial Monitor/Command console

constexpr uint32_t INPUT_TASK_INTERVAL_MS = 1000;   // ms — อ่าน sensor ทุก 1 วิ
constexpr uint32_t CONTROL_TASK_INTERVAL_MS = 1000; // ms — คุม actuator ทุก 1 วิ
constexpr uint32_t COMMAND_TASK_INTERVAL_MS = 50;   // ms — loop เช็ค Serial ทุก 50ms
...
#define DEBUG_TEMP_LOG 1        // อุณหภูมิ / ความชื้น SHT40
...
#define CLI_PREFIX "sm" // Command Line Interface prefix (เช่น "sm mode auto")
